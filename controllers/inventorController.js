...
exports.inventor_list = function (req, res, next) {
  Inventor.find()
    ...
    .exec(function (err, list_inventors) {
      ...
      res.render("inventor_list", {
        title: "Inventor List",
        inventor_list: list_inventors,
      });
    });
};
  ...
  async.parallel(
    {
      inventor: function (callback) {
        Inventor.findById(req.params.id).exec(callback);
      },
      inventors_items: function (callback) {
        Item.find({ inventor: req.params.id }, "name description").exec(
          callback
        );
      },
    },
    function (err, results) {
      ...
      // Successful, so render.
      res.render("inventor_detail", {
        title: "Inventor Detail",
        inventor: results.inventor,
        inventor_items: results.inventors_items,
      });
    }
  );
};
...
exports.inventor_delete_post = function (req, res, next) {
  Inventor.findByIdAndRemove(req.body.id, function (err) {
    if (err) {
      return next(err);
    }
